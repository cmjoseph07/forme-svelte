...
<Tip>
Prefer Tailwind syntax? Use [`@formepdf/tailwind`](/tailwind) to write `tw("flex-row gap-3 p-4 bg-slate-50")` instead of style objects.
</Tip>
...
| `fontFamily` | `string` | `"Helvetica"` | Font family name. Standard PDF fonts: Helvetica, Times, Courier. Register custom TTF fonts with [`Font.register()`](/fonts). |
...
| `wordSpacing` | `number` | `0` | Extra space between words in points (PDF `Tw` operator — affects ASCII spaces). Stacks additively with `text-align: justify` slack |
...
| `background` | `string` | - | CSS gradient or solid color string. Linear: `"linear-gradient(135deg, #667eea, #764ba2)"`. Radial: `"radial-gradient(circle, #10b981, #059669)"`. Multi-stop supported. Solid colors fall through to `backgroundColor`. |
| `opacity` | `number` | `1.0` | Element opacity (0.0 to 1.0). Cascades to children — nested opacities multiply. |
| `boxShadow` | `{ offsetX, offsetY, blur, color }` or `string` | - | Offset drop shadow behind the element. String form: `"2 4 0 #00000033"`. Honors `borderRadius`. `blur` is parsed but rendered sharp in this release. |
...
| `borderRadius` | `number \| { topLeft, topRight, bottomRight, bottomLeft }` | `0` | Corner radius in points. A single number applies to all corners. Also rounds the clip path when `overflow: 'hidden'`. |
