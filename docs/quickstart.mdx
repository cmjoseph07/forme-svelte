...
Forme is a PDF generation library for React and TypeScript. You write JSX components, and Forme renders them to a valid PDF file. There is no headless browser, no Chrome dependency, and no CSS page break hacks. The layout engine runs as a WASM module and produces PDF bytes in milliseconds. Enterprise features include [fillable forms](/forms), [PDF/UA accessibility](/accessibility), [PDF/A archival](/archival), and [digital signatures](/digital-signatures).
...
```bash
npx @formepdf/cli build invoice.tsx -o invoice.pdf
```
...
```bash
npx @formepdf/cli build invoice.tsx --data data.json -o invoice.pdf
```
