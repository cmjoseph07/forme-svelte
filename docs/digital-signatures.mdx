...
description: Apply PKCS#7 digital certifications to PDFs for contracts, compliance, and legal documents.
---

Digital signatures cryptographically prove that a PDF has not been tampered with since certification and identify the certifier. They are required or expected for contracts, regulatory filings, legal documents, and compliance workflows. Forme supports PKCS#7 detached signatures using X.509 certificates, implemented in pure Rust with no external signing service.
...
Use the `/v1/certify` endpoint to apply a digital signature to a PDF you have already rendered or received from another source.
...
The endpoint returns `200 OK` with the certified PDF as the response body (`Content-Type: application/pdf`).

<Note>The deprecated `/v1/sign` endpoint still works and routes to the same handler.</Note>
