...
Yes. Forme can apply PKCS#7 detached digital signatures using X.509 certificates. Certify at render time via the `certification` prop on `<Document>`, or certify an existing PDF via the `/v1/certify` API endpoint. See the [Digital Signatures guide](/digital-signatures).
