...
title: Go SDK
sidebarTitle: Go
description: Generate PDFs from Go with the hosted API or local WASM rendering. Zero-dependency API client, plus a component DSL for building documents in Go code.
---
...
| `Certify(pdf, opts)` | Certify a PDF with X.509 certificate | `([]byte, error)` |
| `Redact(pdf, opts)` | Redact regions/patterns from a PDF | `([]byte, error)` |
| `Rasterize(pdf, opts)` | Convert PDF pages to PNG images | `(*RasterizeResult, error)` |
